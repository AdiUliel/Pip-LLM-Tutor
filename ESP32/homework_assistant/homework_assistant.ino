...
bool runIdentifyFlow(String& firstQuestionOut, String& firstAudioUrlOut) {
  ...
  const char* welcomeText = "היי! מי כאן? תגיד לי איך קוראים לך אחרי שתלחץ על הכפתור.";
  ...
  const char* nameRetry = "לא שמעתי. תלחץ על הכפתור ותגיד שוב איך קוראים לך.";
...
void setup() {
  ...
  static const char* const STATIC_TTS_PHRASES[] = {
    "היי! מי כאן? תגיד לי איך קוראים לך אחרי שתלחץ על הכפתור.",
    "לא שמעתי. תלחץ על הכפתור ותגיד שוב איך קוראים לך.",
  ...
  if (useIdentifyFlow && g_childId.isEmpty()) {
    ...
    faceStrip("");   // clear the "מחובר!" confirmation so it doesn't linger through the identify flow
  }
