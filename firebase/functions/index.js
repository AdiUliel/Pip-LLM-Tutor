 ...
 *
 * Main idea:
 *   Device / Flutter App writes a child answer to:
 *     /sessions/{sessionId}/exchanges/{exchangeId}
 *   with status="pending" and childAnswer="..."
 *
 * The function:
 *   1. Reads the active session and child profile.
 *   2. Checks the answer against the current expected answer.
 *   3. Asks Gemini for a short emotional feedback sentence.
 *   4. Generates the next math/English question.
 *   5. Writes a structured JSON result back to Firestore.
 *
 * Backward compatibility:
 *   If an exchange contains only { question, status:"pending" }, the function
 *   still answers it as a normal homework-helper message, like the original
 *   project did.
 */
...
async function answerFreeTextQuestion(sessionId, exchangeId, data) {
  ...
  // Read session and child profile in parallel for personalization.
  ...
  // Build system instruction personalised for this child.
 ...
 * question immediately. ESP32 can also create the session document directly.
 */
